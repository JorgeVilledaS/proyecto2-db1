...
const soloGerencia = requireRol('rol_admin', 'rol_gerente');

router.get('/ventas-por-categoria', soloGerencia, async (_req, res, next) => {
  try {
...
router.get('/ranking-productos', soloGerencia, async (_req, res, next) => {
  try {
    const result = await pool.query(`
      ...
      SELECT *, RANK() OVER (ORDER BY ingresos_generados DESC) AS ranking
      FROM ventas_por_producto ORDER BY ranking
    `);
    res.json(result.rows);
  } catch (err) { next(err); }
});

router.get('/productos-sin-ventas', soloGerencia, async (_req, res, next) => {
  try {
    const result = await pool.query(`
      ...
      WHERE NOT EXISTS (
        ...
        JOIN venta v ON dv.id_venta=v.id_venta
        WHERE dv.id_producto=p.id_producto AND v.estado='completada'
      )
      ORDER BY p.nombre
    `);
    res.json(result.rows);
...
router.get('/top-clientes', soloGerencia, async (_req, res, next) => {
  try {
    const result = await pool.query(`
      SELECT resumen.id_cliente, resumen.cliente, resumen.email,
             resumen.total_compras, resumen.monto_total, resumen.ultima_compra
      FROM (
        SELECT cl.id_cliente, cl.nombre||' '||cl.apellido AS cliente, cl.email,
               COUNT(v.id_venta)               AS total_compras,
               ROUND(SUM(v.total)::numeric,2)  AS monto_total,
               MAX(v.fecha_venta)              AS ultima_compra
        FROM cliente cl JOIN venta v ON cl.id_cliente=v.id_cliente
        WHERE v.estado='completada'
        GROUP BY cl.id_cliente, cl.nombre, cl.apellido, cl.email
      ) AS resumen
      WHERE resumen.total_compras >= 1
      ORDER BY resumen.monto_total DESC LIMIT 10
    `);
    res.json(result.rows);
  } catch (err) { next(err); }
});

router.get('/rendimiento-empleados', soloGerencia, async (_req, res, next) => {
  try {
    const result = await pool.query(`
      SELECT e.id_empleado, e.nombre||' '||e.apellido AS empleado, e.cargo,
      ...
      FROM empleado e
      LEFT JOIN venta v ON e.id_empleado=v.id_empleado AND v.estado='completada'
...
router.get('/stock-bajo', async (_req, res, next) => {
  // Stock bajo: visible para admin, gerente y bodeguero
  try {
    const result = await pool.query(`
      SELECT p.id_producto, p.nombre AS producto, p.stock AS stock_actual,
             p.precio_unitario, c.nombre AS categoria, pr.nombre AS proveedor,
             ROUND((SELECT AVG(p2.stock) FROM producto p2 WHERE p2.id_categoria=p.id_categoria)::numeric,1) AS promedio_stock_categoria
      FROM producto p
      JOIN categoria c  ON p.id_categoria = c.id_categoria
      JOIN proveedor pr ON p.id_proveedor  = pr.id_proveedor
